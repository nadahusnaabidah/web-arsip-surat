...
{
    try {
        // Cek apakah ada arsip terkait dengan kategori yang akan dihapus
        if (Arsip::where('kategori_id', $id)->exists()) {
            return redirect()->route('kategori')->with('error', 'Kategori tidak dapat dihapus karena masih memiliki arsip terkait.');
        }
        ...
        // Tangkap exception jika terjadi masalah pada query
        return redirect()->route('kategori')->with('error', 'Terjadi masalah saat menghapus data');
    }
}
